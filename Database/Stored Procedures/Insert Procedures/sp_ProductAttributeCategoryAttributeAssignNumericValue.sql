...
begin
    ...
    if not exists (select 1 from CategoryAttributes where Id = @CategoryAttributeId and AttributeType = 2)
        begin
            raiserror ('CategoryAttribute is not of numeric type.', 16, 6);
            return 6;
        end

    insert into ProductAttributes (CategoryAttributeId, ProductId, NumericValue)
    values (@CategoryAttributeId, @ProductId, @NumericValue);
